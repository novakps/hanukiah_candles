...
#define PIN 0              // Which pin are those Neopixels hook up to?
#define NUMBER_OF_FLAMES 18 // depends on number of neopixel triplets. 5 for 16 NeoPixel ring. 4 for 12 NeoPixel ring
#define FLICKER_CHANCE 1   // increase this to increase the chances an individual flame will flicker
...
Adafruit_NeoPixel strip = Adafruit_NeoPixel(NUMBER_OF_FLAMES, PIN, NEO_GRB + NEO_KHZ800);
...
struct flame_element{
 ...
 const int flamecolors[22][3] PROGMEM = {
{ SCALERVAL, 0,  0},
{ SCALERVAL, 0,  0},
{ SCALERVAL, 0,  0},
{ SCALERVAL, 0,  0},
...
void setup() {
  ...
  InitFlames();
  day = EEPROM.read(0) + 1;
  EEPROM.write(0, day % 8);
}

void loop() {

 for(byte flame_count=0; flame_count<(day + 1) * 2; flame_count++) {
...
// 
  ...
  for(byte sub_pixel=0; sub_pixel<1; sub_pixel++) {
    ...
    strip.setPixelColor(flame_num + sub_pixel, c);
  }
...
void CreateNewFlame(byte flame_num){
 ...
 flames[flame_num].max_brightness = GetMaxBrightness();
  ...
  for(byte i=0; i<3; i++) {
    flames[flame_num].rgb[i] = pgm_read_word(&(flamecolors[color_index][i]));
  }
